...
vrr <- read_csv("./data/VRR.csv") %>% 
  select(1:2,) %>% tail(5)
```
Once the best model has been found, we can use it to derive the economic values, which in this case, are the willingness to pay for time saving and risk reduction. 

Table: Estimated Value of Statistical Life, Feb 2024
`r vrr %>% kbl %>% kable_styling()`
